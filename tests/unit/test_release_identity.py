...
def test_v046_release_identity_is_consistent() -> None:
    project = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    assert project["project"]["version"] == "0.4.6"
    assert __version__ == "0.4.6"


def test_v046_readme_trusted_field_baseline_is_current() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "trusted field baseline is **v0.4.6**" in readme
    assert "**v0.4.6 Candidate**" not in readme
