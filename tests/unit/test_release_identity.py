...
def test_v0421_testing_release_identity_is_consistent() -> None:
    project = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    assert project["project"]["version"] == "0.4.21"
    ...
    assert "Version: 0.4.21" in pkg_info


def test_v0421_readme_keeps_v0414_as_trusted_release_baseline() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "trusted field baseline is **v0.4.14**" in readme
    assert "**v0.4.21 Testing**" in readme
    assert "Negative-Condition Monitor" in readme
    assert "trusted field baseline is **v0.4.21**" not in readme
