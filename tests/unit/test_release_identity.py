...
def test_v035_candidate_release_identity_is_consistent() -> None:
    project = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    assert project["project"]["version"] == "0.3.5"
    assert __version__ == "0.3.5"

def test_v035_candidate_readme_release_status_is_current() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "trusted field baseline is **v0.3.3**" in readme
    assert "**v0.3.5 Candidate**" in readme
