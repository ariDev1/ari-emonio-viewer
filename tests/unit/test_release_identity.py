...
def test_v039_candidate_release_identity_is_consistent() -> None:
    project = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    assert project["project"]["version"] == "0.3.9"
    assert __version__ == "0.3.9"

def test_v039_candidate_readme_release_status_is_current() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "trusted field baseline is **v0.3.8**" in readme
    assert "**v0.3.9 Candidate**" in readme
