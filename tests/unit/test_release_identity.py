...
def test_v0423_testing_release_identity_is_consistent() -> None:
    project = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    assert project["project"]["version"] == "0.4.23"
    ...
    assert "Version: 0.4.23" in pkg_info


def test_v0423_readme_keeps_v0414_as_trusted_release_baseline() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "trusted field baseline is **v0.4.14**" in readme
    assert "**v0.4.23 Testing**" in readme
    assert "Negative-Condition Monitor" in readme
    assert "trusted field baseline is **v0.4.23**" not in readme
